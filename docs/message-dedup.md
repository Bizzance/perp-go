...
## 方案：按(topic, partition, offset)落一张"已处理"表
...
`processed_messages`表（`sql/schema.sql`）以这个三元组为主键：
...
`internal/repo.ProcessedMessageRepo.TryMark`处理消息前先`INSERT IGNORE`占位：插入成功
（受影响行数>0）说明是第一次处理，插入被忽略（受影响行数=0，撞了主键）说明这条消息
之前已经处理过。用"先插入、按受影响行数判断"而不是"先SELECT再INSERT"，是因为唯一约束
本身就是数据库层面并发安全的去重屏障，不需要应用层自己加锁应对"查完还没插、另一个
goroutine也在查"这种竞态窗口（虽然这几个Kafka消费者目前都是单goroutine顺序处理各自
topic，暂时不存在这种并发，但这个写法本身更健壮，不依赖"暂时只有一个消费者"这个前提）。
...
- `Message`结构体：`Consumer.Consume`的handler签名从`func(key, value []byte) error`
  ...
  这三个字段被丢弃了，去重需要它们
- `DedupChecker`接口：只要求一个`TryMark`方法，`mq`包不直接依赖`repo`包（避免包依赖
  方向不对——`mq`是更底层的基础设施，不应该知道`repo`这种业务层的存在），`*repo.
  ProcessedMessageRepo`天然满足这个接口
- `WithDedup(ctx, checker, handler)`：返回一个包装过的handler，处理前先`TryMark`，
  已经处理过的直接跳过（返回nil，不执行真正的业务handler），没处理过的正常放行

`cmd/contract-engine/main.go`三个消费者（`order.submit`/`order.cancel`/`round.close`）
统一用`mq.WithDedup(ctx, processedMsgRepo, handler)`包一层，不需要三处各自实现一遍
去重逻辑。
