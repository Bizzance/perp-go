...
- **MARKET类型触发后缺流动性会卡在未成交状态**：这是这个系统里MARKET单的既有行为（普通
  `POST /order/add`提交的市价单同样如此），不是条件单专属的新问题——`SubmitOrder`对
  MARKET单"剩余量不挂簿"，缺对手盘时这部分剩余量就不会再被撮合，委托停留在
  `open`/`partially_filled`状态不会再变化。
