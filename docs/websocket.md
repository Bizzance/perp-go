...
**`user`频道不会因为标记价变化而推送，持仓盈亏要客户端自己按标记价算。** `user`频道只在下单、成交、撤单、强平、结束本轮这类
事件发生时推一次账户、持仓、挂单。标记价每秒都可能变，但不会触发`user`推送，所以持仓里的`markPrice`、`unrealizedPnl`、`roe`是"上一次事件时"
算的值。想让盈亏跟着标记价实时变，订阅`markprice:{symbol}`频道，按同样的公式重算（`tools/sim-client/web/app.js`的`recomputeForMark`是参考实现）：
多头`未实现盈亏=(标记价-开仓均价)×数量`，空头反过来；`回报率=未实现盈亏/仓位保证金`；账户权益的变化量等于各仓位未实现盈亏变化量之和。
服务端不按标记价变化推送，是因为每个有持仓的用户每秒一条推送，用户多了开销太大，而这个值客户端用已有的数据就能算。下一次`user`推送或REST查询会用
服务端算好的值整体覆盖。
