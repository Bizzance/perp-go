...
- **指数价只有币安一个来源**：orderbook-sync 把币安算好的指数价（币安自己按多家现货交易所加权得出）推给 contract-api，标记价靠它做锚，
  没有多来源交叉校验。`POST /index-price` 有服务端跳变保护（一次变动超过`PERP_INDEX_MAX_JUMP`要新价位持续几秒才承认，默认不开、
  生产建议设0.05），但拿着ops密钥的人每次只挪一小步它挡不住，所以密钥要单独发。币安不可访问（部分地区返回451）或指数价断供时标记价冻结、
  强平/资金费率/条件单暂停，恢复后一步跳到新价没有平滑，见 [mark-price.md](mark-price.md)、[orderbook-sync.md](orderbook-sync.md)。
- **盘口太薄时资金费率采不到溢价样本**：溢价用冲击价格算（见 [funding-rate.md](funding-rate.md)"溢价指数"），任何一侧盘口的总名义价值
  不到合约的`funding_impact_notional`（默认10000 USDT）就跳过这个合约这一轮采样，一个周期一个样本都没有资金费率就是0。
  冲击名义金额要按合约调、且要小于盘口深度；orderbook-sync没在跑（或币安数据过期被撤光挂单）时盘口是空的，资金费率也就一直是0。
  另外这个字段是新加的，已经建过库的环境要重建库。
...
- **系统是唯一对手方，不对冲，没有敞口限额**：订单簿同步（[orderbook-sync.md](orderbook-sync.md)）把币安的盘口挂进我们的订单簿，用户下单吃的就是系统的挂单。
  用户整体盈利时系统亏损没有上限保护，系统账户余额是配置的虚拟资金。结束本轮、强平超时兜底、ADL 这三条路径按标记价和系统成交，没有对手方记账，
  系统的敞口在账上看不见
- **K线、24h统计是币安的数据**：生产环境（`PERP_KLINE_SOURCE=external`）K线不再用我们自己的成交生成，成交量和成交笔数是币安全市场的，不是我们平台的；
  24h统计是最近24根1小时K线聚合，跟币安滚动24h行情有约1%的差异。**把币安行情数据再分发给合作方是否合规待确认**，见 [orderbook-sync.md](orderbook-sync.md)
- **持仓盈亏不随标记价推送**：`user` 频道只在下单/成交/撤单/强平这些事件时推持仓，标记价变化不触发，客户端要订阅 `markprice` 频道自己重算，见 [websocket.md](websocket.md)
- **盘口比币安有延迟**：REST 轮询每秒一次，行情快的时候我们的最优价会落后币安（实测平均约0.003%，最大约0.026%），
  谁比我们更早看到币安的价格，谁就有套利空间。降低它靠缩短间隔、以后改用 WebSocket
- **已经建过库的环境，合约的价格步进和数量步长要手动配**：`sql/schema.sql`重复执行不会覆盖已有合约（`ON DUPLICATE KEY UPDATE`是空操作），
  已有环境里BTC/ETH的`price_tick`/`volume_step`还是0（=服务端不校验），要自己`UPDATE coins SET price_tick=…, volume_step=… WHERE symbol=…`，
  数值见`sql/schema.sql`的种子数据。新合约不配也是不校验，对接方按`/contract/list`显示位数时会跟服务端实际接受的不一致
- **测试环境的两个遗留**（只影响测试环境）：模拟客户端的界面冒烟脚本要不要放进仓库（还没决定）；模拟客户端做市的系统账户会累积仓位
  （多空各一份、不轧差），目前不会自动处理，要手动点"重置系统账户"
