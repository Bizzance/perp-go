...
- **生产环境的对手方和对冲**：产品设定是上线后用户的对手方就是系统，但生产环境目前**没有任何做市方**——模拟客户端
  （`tools/sim-client`）的系统做市只是测试工具，不进生产镜像，见 [sim-client.md](sim-client.md)。没有对手方，生产的订单簿里就没有流动性。
  这块需要先出设计方案再做：生产的对手方是谁、系统持仓怎么对冲（要不要在币安等交易所真实对冲）、亏损谁来承担。
