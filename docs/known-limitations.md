...
- **标记价的喂价源是单点，且没有校验**：标记价靠`POST /index-price`喂进来的指数价做锚，目前只有一个来源，
  没有多交易所取中位数的喂价适配器，也没有单次跳变幅度校验；喂价源出错标记价就跟着错。指数价断供时标记价冻结、
  强平/资金费率/条件单暂停，恢复后一步跳到新价没有平滑。上线前要补多源喂价，见 [mark-price.md](mark-price.md)。
