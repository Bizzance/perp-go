...
- **标记价的锚(指数价)的服务端保护只限制变动速度**：`POST /index-price`有跳变保护（一次变动超过`PERP_INDEX_MAX_JUMP`
  要新价位持续几秒才承认，默认不开、生产建议设0.05），但没有离群校验，拿着ops密钥的人每次只挪一小步就挡不住，
  所以密钥要单独发；离群和多来源取中位数靠index-feeder，见 [index-feeder.md](index-feeder.md)、[mark-price.md](mark-price.md)。
...
- **生产环境的对手方、大单和价格跟随**：产品的设定是引入币安的行情和订单簿，用户下单的对手方就是系统，没有做市商，也不对冲，
  价格要跟着币安走。但生产环境目前**没有任何流动性来源**——模拟客户端（`tools/sim-client`）的系统做市只是测试工具，不进生产镜像，
  见 [sim-client.md](sim-client.md)，它镜像的深度很浅（约0.07%），也没有大单护栏。这块需要先出设计方案再做：一次吃掉很大深度的大单怎么
  不让价格偏离币安、系统作为唯一对手方的敞口和亏损怎么封顶、行情延迟怎么防（用户能比我们更早看到币安价格就可以套利）。
  方案草案和待决策事项见 [counterparty-hedging-design.md](counterparty-hedging-design.md)：注意结束本轮、强平超时兜底、ADL这三条路径
  现在就是"按标记价和系统成交"，但没有对手方记账，系统的敞口在账上看不见。
