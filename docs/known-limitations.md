...
- **盘口太薄时资金费率采不到溢价样本**：溢价用冲击价格算（见 [funding-rate.md](funding-rate.md)"溢价指数"），任何一侧盘口的总名义价值
  ...
  冲击名义金额要按合约调、且要小于盘口深度；订单簿镜像没配置（或币安数据过期被撤光挂单）时盘口是空的，资金费率也就一直是0。
...
- **系统是唯一对手方，不对冲，没有敞口限额**：订单簿镜像（[orderbook-sync.md](orderbook-sync.md)）把币安的盘口挂进我们的订单簿，用户下单吃的就是系统的挂单。
...
- **orderbook-sync挂单堆积、把数据库拖垮**：本地测试环境实测复现过——早期版本用独立进程通过公开下单/撤单API+
  分布式锁镜像币安订单簿，一个系统账户的挂单数堆到2000+笔（最初是一批历史挂单撤不掉），MySQL被拖到CPU 145%，
  之后每一轮同步都在"跟这一大批旧挂单逐笔比对+抢账户锁挂新单"上继续变慢、继续失败，订单簿卡住不再更新
  （K线走的是另一条推送路径，所以还在动，容易被误判成"只有订单簿的问题"）。根子问题是套用了为多进程并发
  设计的分布式锁和异步撤单确认，但镜像挂单从来只有一个来源。后来把这部分整个搬进了`contract-engine`进程
  内部直接调用（不再经HTTP/Kafka/分布式锁），撤单变成同步生效，这类堆积从架构上不再可能出现，
  见 [orderbook-sync.md](orderbook-sync.md)。
...
- **合作方对接接口的几处缺陷（对接评审发现）**：① **响应JSON字段命名混乱**：模型结构体没有`json`
  ...
  `round`不等于账户当前`round`就忽略）两层防护，用真实数据验证过：结束第1轮后进入第2轮、发额度开仓，
  再重试结束第1轮，第2轮的持仓和额度原样保留；直接往Kafka发一条过期事件，引擎日志打忽略警告、状态不变。这一组问题的共同教训：
