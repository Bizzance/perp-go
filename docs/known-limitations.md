...
- **没有鉴权**：全系统靠明文`uid`，包括运营类接口（加钱扣钱、发额度、喂指数价）和WS私有频道。
  这是对接前必须补的一项，方案见 [auth-design.md](auth-design.md)，尚未实现。
- **没有账户状态（冻结/封禁）**：账户只有"存在"和"不存在"两种，没法冻结或封禁某个用户。有真实需求时
  再加`status`字段和对应校验，创建接口是天然的挂载点。
- **没有限流**：任何接口都没有按调用方限流，见 [auth-design.md](auth-design.md)"其它配套"。
