...
- **强平后的结算不考虑挂单冻结的保证金**：`HandleLiquidationSettleAftermath`只看`available + credit`。如果用户
  强平时还有挂着的开仓委托，`available`被亏成负数时基金会照常垫付，之后撤单把冻结的保证金退回账户，用户实际拿到
  的比真实权益多出这部分垫付（探针复现过：`available=-100`、挂单冻结600，基金垫付100，撤单后账户有600，真实权益
  本来是500、没有缺口）。强平触发时先撤掉这个uid的全部挂单（真实交易所的做法）可以一并解决，还没做。
