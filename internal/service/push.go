...
// 通过Redis Pub/Sub往WS网关(contract-api的internal/ws.Hub)发布实时事件——
...
// 把payload序列化成JSON发到指定channel，失败只记日志——推送是锦上添花的实时通知，
...
func (p *PushService) PublishTrade(ctx context.Context, trade *model.Trade) {
	// 公开频道，只推公开视图，不能带买卖双方uid/委托id
	p.publish(ctx, pubsub.TradeChannel(trade.Symbol), trade.Public())
}
...
// 私有频道user:{uid}推送的内容——账户/持仓/当前挂单三合一，不是增量diff。
...
// 重新查一遍这个uid当前的account/positions/active orders，整体推送。
...
func (p *PushService) PublishUserSnapshot(ctx context.Context, uid uint64) {
	...
	// nil切片会被序列化成JSON的null，客户端遍历null会出错，"没有挂单"必须是空数组
	if orders == nil {
		orders = []model.Order{}
	}
	if positions == nil {
