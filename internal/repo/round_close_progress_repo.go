...
// 结束本轮(CloseRound)在engine分片部署下的跨分片完成度追踪，见
...
// 给(uid,round)涉及到的symbol占一行坑(done=0)——用INSERT IGNORE，不是SELECT-then-INSERT：
...
// 把某个symbol的这部分标记为完成——只有真正拥有这个symbol的分片实例才会调用，
...
// 这个(uid,round)涉及到的全部symbol是不是都完成了——一行都没seed过(uid在这个round
...
// 最终结算完成后清理掉这个(uid,round)的全部进度行，不然这张表会无限增长
